...
def cudavec(x, grad=False, gpu_id=-1):
  if gpu_id == -1:
    return var(torch.from_numpy(x), requires_grad=grad)
  else:
    return var(torch.from_numpy(x).pin_memory(), requires_grad=grad).cuda(gpu_id, async=True)


def cudalong(x, grad=False, gpu_id=-1):
  if gpu_id == -1:
    return var(torch.from_numpy(x.astype(np.long)), requires_grad=grad)
  else:
    return var(torch.from_numpy(x.astype(np.long)).pin_memory(), requires_grad=grad).cuda(gpu_id, async=True)
