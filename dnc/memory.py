...
class Memory(nn.Module):
	def __init__(self, input_size, mem_size=512, cell_size=32, read_heads=4, gpu_id=-1, independent_linears=True):
		...
		if self.independent_linears:
			...
			self.I = cuda(1 - torch.eye(m).unsqueeze(0), gpu_id=self.gpu_id)  # (1 * n * n)
	...
	def reset(self, batch_size=1, hidden=None, erase=True):
		...
		if hidden is None:
			return {
				'memory': cuda(torch.zeros(b, m, w).fill_(0), gpu_id=self.gpu_id),
				'link_matrix': cuda(torch.zeros(b, 1, m, m), gpu_id=self.gpu_id),
				'precedence': cuda(torch.zeros(b, 1, m), gpu_id=self.gpu_id),
				'read_weights': cuda(torch.zeros(b, r, m).fill_(0), gpu_id=self.gpu_id),
				'write_weights': cuda(torch.zeros(b, 1, m).fill_(0), gpu_id=self.gpu_id),
				'usage_vector': cuda(torch.zeros(b, m), gpu_id=self.gpu_id)
			}
		else:
	...
	def allocate(self, usage, write_gate):
		...
		# free list
		sorted_usage, φ = torch.topk(usage, self.mem_size, dim=1, largest=False)
		...
		_, φ_rev = torch.topk(φ, k=self.mem_size, dim=1, largest=False)
	...
	def write(self, write_key, write_vector, erase_vector, free_gates, read_strengths, write_strength, write_gate, allocation_gate, hidden):
		...
		hidden['memory'] = hidden['memory'] + \
			torch.bmm(hidden['write_weights'].transpose(1, 2), write_vector)
